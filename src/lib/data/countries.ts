/**
 * ISO 3166-1 alpha-2 country codes with English short names and demonyms.
 * Sorted alphabetically by country name. Used for the country / nationality /
 * residence dropdowns in the User Panel. The `code` is what we persist
 * (matches the schema convention — `country`, `nationality`, and
 * `country_of_residence` columns all store the 2-letter code).
 *
 * `name` is the country name (e.g. "France") — used for the country and
 * country-of-residence dropdowns.
 * `nationality` is the English demonym (e.g. "French") — used for the
 * nationality dropdown. Both map to the same ISO code under the hood.
 */
export interface Country {
  ...
  name: string;
  /** English demonym (the person-noun for someone of this nationality) */
  nationality: string;
}
...
export const COUNTRIES: readonly Country[] = [
  ...
  { code: 'CG', name: 'Congo (Brazzaville)', nationality: 'Congolese' },
  { code: 'CD', name: 'Congo (Kinshasa)', nationality: 'Congolese' },
  ...
  { code: 'TR', name: 'Türkiye', nationality: 'Turkish' },
...
/** Lookup the country name for a stored code, or the code itself if unknown. */
...
/** Lookup the demonym for a stored code, or the code itself if unknown. */
export function countryNationality(code: string | null | undefined): string {
  if (!code) return '';
  const found = COUNTRIES.find((c) => c.code === code);
  return found?.nationality ?? code;
}

/**
 * Countries sorted by demonym (since the nationality dropdown displays
 * demonyms, not country names). The country/residence dropdowns continue to
 * use the default name-sorted order via `COUNTRIES`.
 */
export const COUNTRIES_BY_NATIONALITY: readonly Country[] = [...COUNTRIES].sort(
